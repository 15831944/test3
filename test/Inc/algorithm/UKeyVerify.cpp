...
BOOL CUKeyVerify::GetObjectValue(CK_BYTE *pUserData, CK_ULONG *pUserDataLen)
{
	...
	do 
	{
		...
		if (dataAttr[3].pValue == NULL || dataAttr[3].ulValueLen == 0)
		{
			bRet = FALSE;
			break;
		}

		if (*pUserDataLen < dataAttr[3].ulValueLen)
		{
			bRet = FALSE;
			break;
		}

		*pUserDataLen = dataAttr[3].ulValueLen;
		memcpy(pUserData, dataAttr[3].pValue, dataAttr[3].ulValueLen);
