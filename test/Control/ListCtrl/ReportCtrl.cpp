...
void CReportCtrl::OnLButtonUp(UINT nFlags, CPoint point)
{
	...
	ReleaseCapture();
	_MouseClkMonitor(WM_LBUTTONUP, nFlags, point, TRUE);
}

void CReportCtrl::OnMouseLeave()
{
	m_bTracking = FALSE;
	m_bMouseDown = FALSE;
...
void CReportCtrl::OnMouseMove(UINT nFlags, CPoint point)
{
	...
	do 
	{
		if (rcClient.PtInRect(point))
		{
			if (m_bMouseDown)
			{
				...
				double nMax = GetScrollLimit(SB_VERT);

				double dfVar = nMax;
				double dfHeight = rcClient.Height();

				if (point.y > m_ptMouse.y)
				{//向下滑动
					//TRACE("%d-%d", m_ptMouse.y, point.y);
					nThumbTop = m_ptMouse.y + point.y;
				}
				else
				{//向上滑动
					nThumbTop = m_ptMouse.y - point.y;
				...
				if (nThumbTop < rcItem.Height())
				{
					break;
				}

				dfThumbInterval = dfHeight/dfVar;
				nScrollTimes  = (int)(nThumbTop/dfThumbInterval)-nPos;

				size.cx = 0;
				size.cy = nScrollTimes*rcItem.Height();
	...
	_MouseClkMonitor(WM_MOUSEMOVE, nFlags, point, FALSE);
}
